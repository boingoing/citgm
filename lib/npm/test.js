...
function test(context, next) {
  ...
  readPackage(path.join(wd, 'package.json'), false, function(err, data) {
    ...
    if (data.dependencies['node-addon-api'] && process.versions.modules >= 52) {
      // A dependency on the N-API package was detected, and the Node version supports N-API.
      // So enable N-API modules when running tests.
      options.env.NODE_OPTIONS = '--napi-modules';
    }
